...
namespace EnterpriseMaster.DesktopApp.Data.Services.AnalyticsServices
{
    public class AnalyticsService
    {
        ...
        public async Task<List<Employees>> GetAllEmployeesAsync()
        {
            try
            {
                return (await employeesServices.GetAllAsync()).Where(item => item.IsActive == true).ToList();
            }
            catch (Exception e)
            {
                await errorLogsServices.AddAsync(new ErrorLogs() { Date = DateTime.Now, Message = e.Message, Exception = e.StackTrace });
        ...
        public async Task<List<ProductionOrders>> GetAllProductionOrdersAsync()
        {
            try
            {
                return (await productionOrderServices.GetAllAsync()).Where(item => item.IsActive == true).ToList();
            }
            catch (Exception e)
            {
                await errorLogsServices.AddAsync(new ErrorLogs() { Date = DateTime.Now, Message = e.Message, Exception = e.StackTrace });
        ...
        public async Task<List<Tasks>> GetAllTasksAsync()
        {
            try
            {
                return (await tasksServices.GetAllAsync()).Where(item => item.IsActive == true).ToList();
            }
            catch (Exception e)
            {
                await errorLogsServices.AddAsync(new ErrorLogs() { Date = DateTime.Now, Message = e.Message, Exception = e.StackTrace });
